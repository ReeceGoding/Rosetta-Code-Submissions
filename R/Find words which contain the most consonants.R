...
singleConstWords <- dictBig[apply(consonantCount, MARGIN = 1, function(x) all(x <= 1))]
uniqueConstCount <- rowSums(sapply(consonants, function(x) grepl(x, singleConstWords)))
biggestWords <- function(offset) singleConstWords[which(uniqueConstCount == max(uniqueConstCount) - offset)]
